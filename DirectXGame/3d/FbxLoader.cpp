...
Model* FbxLoader::LoadModelFromFile(const string& modelName) {
	...
	PareseNodeRecursive(model, fbxScene->GetRootNode());
	//FBXシーン解放
...
void FbxLoader::PareseNodeRecursive(Model* model, FbxNode* fbxNode, Node* parent) {
	...
	node.transform += matTranslation;   //ＷＯＲＬＤ業レ悦に平行移動を反映
	...
	for (int i = 0; i < fbxNode->GetChildCount(); i++) {
		PareseNodeRecursive(model, fbxNode->GetChild(i));
	}
}
