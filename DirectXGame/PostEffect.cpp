...
void PostEffect::CreateTexture(HRESULT& result)
{
	//テクスチャリソース設定
...
void PostEffect::CreateSRV(HRESULT& result)
{
	//SRV用デスクリプタヒープ設定
...
void PostEffect::CreateRTV(HRESULT& result)
{
	//RTV用デスクリプタヒープ設定
	...
	//デスクリプタヒープにRTV生成
	device->CreateRenderTargetView(texBuff.Get(), &renderTargetViewDesc,
		descHeapRTV->GetCPUDescriptorHandleForHeapStart());
}

void PostEffect::CreateDepthBuffer(HRESULT& result)
{
	//深度バッファリソース設定
...
void PostEffect::CreateDSV(HRESULT& result)
{
	//DSV用デスクリプタヒープ設定
